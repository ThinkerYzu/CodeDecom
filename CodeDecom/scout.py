...
class Scout(object):
    '''A Scout is a proxy of an instruction and the value generated by the instruction.

    A Scout is a proxy of an instruction and value.  It is used to
    trace the control flow and the data flow of a function. Each
    instruction is supposed to operate on values generated by other
    instructions or passed in from the caller. By monitoring the
    operations on the Scout; the value, we learn the instructions
    being performed and create new instance of Scout to reprsent the
    instruction and the gnerated value.
