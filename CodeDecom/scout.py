'''Scout is a mock of the value generated by an instruction or passed in from the caller.

This module implements a tracer based on the Scout class. (See Tracer
for details)
...
class Scout(object):
    '''A Scout is a mock of the value generated by an instruction.

    A Scout is used to trace the control flow and the data flow of a
    function. Each instruction is supposed to operate on values
    generated by other instructions or passed in from the caller. By
    monitoring the operations on the Scout; a mock value, we learn the
    instructions being performed and create new instance of Scout to
    reprsent the instruction and the gnerated value.

    Scouts for arguments of a function call has their ip set to -1 -
    arg_index.  The arg_index is the index of the argument in the
    argument list.
...
class EnclosingLoopTracer(object):
    '''Trace loops and branches for Tracer.'''
    ...
    def _immediate_enclosing_loop_head(self, insn):
    ...
    def _immediate_2nd_enclosing_loop_head(self, insn):
        for h_ip in insn.enclosing_loop_heads:
            head = self._get_insn(h_ip)
    ...
    def mark_new_enclosing_loop(self, ip):
        ...
        This function is called for each instruction when found.
        ...
        for i , tip in enumerate(reversed(trace_log)):
            ...
            if tip in insn.enclosing_loop_heads:
                # The head of an existing enclosing loop
                break
            pass
    ...
    def find_loop_exits(self):
        '''Find all exit instructions of loops.
            ...
            for i in range(2):
                if insn.br[i] >= 0:
                    ...
                    if br_depth < depth:
                        ...
                        loop_head = self._immediate_enclosing_loop_head(insn)
    ...
    def loop_head_depth(self, loop_head_ip):
        if loop_head_ip == -1:
            return 0
        head = self._get_insn(loop_head_ip)
        return head.loop_depth()

    def compute_immediate_enclosing_loop(self):
        ...
        for insn in self.insns.values():
            if isinstance(insn, list):
                for sub in insn:
                    sub.enclosing_loop_head = self._immediate_enclosing_loop_head(sub)
                    sub.parent_loop_head = self._immediate_2nd_enclosing_loop_head(sub)
                    pass
                pass
            else:
                insn.enclosing_loop_head = self._immediate_enclosing_loop_head(insn)
                insn.parent_loop_head = self._immediate_2nd_enclosing_loop_head(insn)
                pass
            pass
        pass

    def establish_loop_forward_tree(self):
        '''Build the tree of branch instructions'''
        ip = self._first_ip()
        insn = self._get_insn(ip)
        works = [(insn, None)]
    ...
    def show_loop_tree(self):
        ...
        for ip in ips:
            ...
            for ip in subips:
                insn = self._get_insn(ip)
                depth = self.loop_head_depth(insn.enclosing_loop_head)
                sps = '    ' * depth
...
class NamespaceTraceMixin(object):
    '''Trace changes of namespaces.

    The purpose of tracing changes of namespaces is to detect
    assignments to variables and try to list potential variables that
    provide arguments to an instruction.

    '''
    class InsnMixin(object):
        '''Mixin for instructions to hold data for NamespaceTraceMixin.'''
        def __init__(self, *args, **kws):
        ...
        def _current_locals(self, bnav):
            ...
            bnav is the branch navigator.  It is used to get the
            current value of the boolean instruction. Every boolean
            instruction is potentially followed by a branch instruction.
    ...
    def _reset_ns(self):
        # The content of local and global namespaces before running
    ...
    def trace_namespace(self, frame):
        '''Trace the assignment made by the last instruction.

        Checks changes of local namespace and global namespace to
        found assignments.

        Assignments will be performed between the instructions that
        operate values and are monitored by Scout. So, this function
        is called to find the assignments attributed to the
        instruction represented by the last Scout.
    ...
    def trace_arg_srcs(self, frame, *args, **kws):
        '''Collect possible variable names of the arguments of an instruction.

        One value may be assigned to multiple variables. So, the value
        of an argument of an instruction can be from any one of them.
...
class Tracer(EnclosingLoopTracer, NamespaceTraceMixin):
    '''Trace the control flow and data flow of a function.

    The basic idea of CodeDecom is replacing input values, including
    constants and global variables, with Scout instances; mock objects
    of the values. All instructions, except branches, are supposed to
    operate on values. By monitoring the operations on the Scout
    instances, we can learn the instructions being performed and
    create more instances of Scout to represent the generated values.

    For branch instructions, they always have two branches. They
    decide which branch to go based on a boolean value which should be
    mocked by Scot.  Getting boolean value is an operation on a value
    in Python so that the tracer knows the existence of a branch
    instruction.  Manipulating the boolean value of a Scout will
    change the control flow of the function to explore the
    instructions in different paths. Calling a function multiple times
    together with manipulating boolean value of Scout instances can
    collect every instructions and all potential data flows in a
    function.

    An Insn represent an instruction.  When a Scout is created, an
    coressponding Insn is created.  Scout is created for monitoring
    operations on values while Insn is created for recording
    instructions being performed and the data flow.

    ip of an Insn is not exactly the same as raw ip from Python
    frames. Some instruction may cause several operations on values.
    We model them as sub instructions.  The ip of a sub instruction is
    the raw ip of the instruction times 10 plus an offset.  The first
    sub instruction has an offset 0.  The second sub instruction has
    an offset 1.  The N-th sub instruction has an offset N-1.  So, for
    every ip, it can have at most 10 sub instructions. (See
    Tracer._conv_sub_ip())

    For being easier to manage, different aspects of functionalities
    are implemented in separated calsses as mixins.
     - EnclosingLoopTracer
     - NamespaceTraceMixin
    ...
    def __init__(self):
        ...
        self.insns = {}         # ip -> Insn
        self.lasti = -1         # The last found instruction
        self.branch_navi = BranchNavigator(self)
        self.consts = {}        # const value -> Scout
        self.ip_consts = {}     # ip -> const value
        self.extra_globs = {}   # Registeration of the builders of
                                # customized Scouts.
        pass
    ...
    def reg_global(self, name, value):
        '''Register a Scout builder for a global variable.'''
    ...
    def _conv_sub_ip(self, scout):
        '''Convert ip of a Scout instance to sub ip.

        The sub ip is the raw times 10 adding an offset.
    ...
    def found_insn(self, scout):
        '''Record a new instruction found.

        This function is called by Scout to record instructions and
        related information. It also connects previous instruction to
        this just found instruction and setting boolean value.
    ...
    def _plan_guided(self):
        '''Make a path plan to guide the moch function to explore not yet
        visited (dark) branches.

        Every branch instruction has two branches. The tracer can
        explore only one of two branches of an instruction a
        time. This function make a plan to guide the tracer to explore
        new branches.

        This function return Ture if it has a plan or return False if
        have not any branch to explore.
    ...
    def _plan_guided_loops(self):
        '''Generator of lists of branch decisions.

        A generated list of branch decisions can be passed to
        BranchNavigatorGuided to guide the trace to explore all path
        combinations.

        Every loop in a plan should be repeated at least twice in the
        plan to feed the data generated by the loop back to the same
        loop as the input in order to collect a complete set of
        potential inputs for every instruction.
    ...
    def trace(self, func):
        ...
        self.compute_immediate_enclosing_loop()

        self.establish_loop_forward_tree()
    ...
    def debug_show(self):
        ...
        sps = lambda: '    ' * self.loop_head_depth(insn.enclosing_loop_head)
